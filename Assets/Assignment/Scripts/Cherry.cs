...
public class Cherry : MonoBehaviour
{
    ...
    void Start()
    {
        //set starting position + get components
    ...
    void Update()
    {
        ...
        if (points.Count > 0)
        {
            ...
            {
                points.RemoveAt(0);
                for (int i = 0; i < lineRenderer.positionCount - 1.5f; i++)
                {
    ...
    //when cherry is fed to the character, destroy gameObject
    private void OnTriggerStay2D(Collider2D collision)
    {
        collision.gameObject.SendMessage("Happy", 1, SendMessageOptions.DontRequireReceiver);
    ...
    //gameObject follows the movement of the mouse
    ...
    //destroy cherry if it disapearrs off-screen
